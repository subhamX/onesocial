...
import {
  ...
  setTmpAuthTokenAsCookie,
} from "./setSignedTokenAsCookie";
...
export const handleSocialSignInData = async (
  ...
  const { email, name, avatar_url, using_google_auth, using_twitter_auth } =
    userData;
  ...
  const userInstance = await userModelRepository
    .search()
    .where("email")
    .equal(email)
    .return.first();
  ...
  if (!userInstance) {
    const oldEntityIfExists = await potentialUserModelRepository
      .search()
      .where("email")
      .equal(email)
      .return.first();
    ...
    if (userData.twitter_user_name)
      entity.twitter_user_name = userData.twitter_user_name;
    ...
    if (userData.twitter_user_name)
      userInstance.twitter_user_name = userData.twitter_user_name;
