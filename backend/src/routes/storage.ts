  ...
  async function (req, res, next) {
    try {
      ...
      const val: any = await new Promise((resolve, reject) => {
        const cloudinaryStream = uploadDriver.uploader.upload_stream(
          {},
...
app.post(
  ...
  async function (req, res, next) {
    try {
      ...
      if (listing.listing_type !== ListingType.DigitalProduct)
        throw new Error("This listing is not a digital product");
      if (listing.number_of_product_items >= 10)
        throw new Error(
          "You can only add 10 product items to a digital product. Try uploading a zip instead."
        );
...
app.get("/getProduct/:buyInstanceId/:productItemId", async (req, res) => {
  try {
    ...
    const credsJson = fs.readFileSync(
      path.resolve(__dirname, "..", "..", "gcloud-service-account-key.json"),
      { encoding: "utf-8" }
    );
