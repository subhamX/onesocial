...
import {
  DASHBOARD_URL,
  DETAILED_LISTING,
  POST_ADD_LISTING_PRODUCT_ITEMS,
...
const editListingProductItemsMetaData = gql`
  mutation editListingProductItemsMetaData(
...
const ManageContent = () => {
  ...
  const { loading, data, refetch } = useQuery<
    Query,
    QueryGetListingInfoByIdArgs
  >(getListingInfoByIdOnlyProducts, {
    variables: {
  ...
  const [updateMetadataMutateFunction] = useMutation<
    Mutation["editListingProductItemsMetaData"],
    MutationEditListingProductItemsMetaDataArgs
  >(editListingProductItemsMetaData);
  const [deleteFileMutationFunction] = useMutation<
    Mutation["deleteListingProductItem"],
    MutationDeleteListingProductItemArgs
  >(deleteListingProductItem);
  const [publishProductListingMutationFunction] = useMutation<
    Mutation["publishProductListing"],
    MutationPublishProductListingArgs
  ...
  const handleFileUpload = async (ev: ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      if (files.length > 10) {
        alert(
          "You can upload at max 10 files. Consider bundling them into a zip maybe?."
        );
        return;
      }
      setIsUploading(true);
  ...
  if (productData?.listing_type === ListingType.Service) {
    toast.error(
      "This listing is a service listing. It doesn't have any content."
    );
    ...
    if (productData?.number_of_product_items === 0) {
      ...
      publishProductListingMutationFunction({
        ...
        onCompleted(data) {
          ...
          router.push(
            DETAILED_LISTING(productData.listing_type, productData.id)
          );
        },
        onError(error) {
          toast.error(getErrorMessageFromApolloError(error));
      ...
      <div className="max-w-3xl mx-auto mt-5 mb-10 px-5">
        ...
        {productData?.name && (
          <div>
            ...
            <div className="font-extrabold text-lg pb-1 mb-4">
              {productData.name}
            </div>
          </div>
        )}

        {productData?.number_of_product_items !== undefined && (
          <div>
            <div className="font-semibold text-base">
              Number of product items:
            </div>
            <h2 className="font-extrabold text-lg pb-1 mb-4">
              {productData.number_of_product_items}
            </h2>
          </div>
        )}

        {!!productData?.product_items.length && (
          <Formik
            onSubmit={handleSubmit}
            enableReinitialize
            initialValues={{ productItems: productData?.product_items ?? [] }}
          >
            {({ values, dirty }) => (
              <>
                      ...
                      <>
                        {values.productItems.map((e, indx) => {
                          return (
                            <div
                              key={e.id}
                              className="relative text-sm bg-gray-100 py-5 my-2 px-3 border rounded-md items-center gap-5 flex flex-col sm:flex-row w-full"
                            >
                              ...
                              <button
                                className="absolute right-3 -top-2 btn btn-accent text-accent-content btn-xs text-2xs"
                                type="button"
                                onClick={() => handleListingProductDelete(e.id)}
                              >
                                Delete
                              </button>
                            </div>
        ...
        {productData?.product_items.length === 0 && (
          <div className="alert alert-warning mb-4">
            No product items found for this listing. Let&apos;s upload some by
            clicking below
          </div>
        )}
          ...
          <>
            <div className="flex justify-end mt-4">
              {!productData?.is_published ? (
                ...
                <Link
                  href={DETAILED_LISTING(
                    productData.listing_type,
                    productData.id
                  )}
                >
                  <button className="btn btn-secondary">
                    See Listing Public View
                  </button>
                </Link>
              )}
            </div>
