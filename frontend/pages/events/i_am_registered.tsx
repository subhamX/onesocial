...
const EventsWhereIAmRegistered = () => {
  const { data, loading } = useQuery<Query, QueryGetEventsRegisteredArgs>(
    getEventsRegistered,
    {
      variables: {
  ...
  return (
    <div>
      ...
      <div className="max-w-2xl mx-auto px-4">
        ...
        {data?.getEventsRegistered &&
          !!data?.getEventsRegistered.length &&
          data?.getEventsRegistered.map((e, indx) => (
            <Link key={indx} href={DETAILED_EVENT(e.event_id)}>
                ...
                <div className="flex flex-col">
                  <div className="text-base font-bold">Event: {e.title}</div>
                  <div className="text-xs text-gray-500">
                    Start Time:{" "}
                    {new Date(
                      parseFloat(e.event_start_time) * 1000
                    ).toLocaleString()}
                  </div>
