      ...
      <div className="max-w-4xl px-4 mx-auto">
        <div className="flex justify-between mt-5">
          ...
          <Link href={EDIT_EVENT(eventId)}>
            <button className="btn btn-sm normal-case btn-secondary mt-2">
              Edit Event Details
            </button>
          </Link>
        </div>
        ...
        {data?.getRegisteredGuestsInEvent &&
          !!data?.getRegisteredGuestsInEvent.length &&
          data?.getRegisteredGuestsInEvent.map((e, indx) => (
...
const SendMessageToGuests = ({ eventId }: { eventId: string }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-1">
      ...
      <textarea
        className="textarea w-full max-w-lg rounded-none textarea-bordered"
        onChange={(e) => setMessage(e.target.value)}
      />
      <button
        className="mt-2 btn btn-sm normal-case w-fit"
