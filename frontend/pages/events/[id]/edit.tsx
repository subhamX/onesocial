...
const NewEvent = () => {
  ...
  const { loading, data } = useQuery<Query, QueryGetEventInfoByIdArgs>(
    getEventInfoById,
    {
      variables: {
    ...
    mutateFunction({
      variables: {
        payload: {
          ...
          duration_in_minutes: Math.round(
            (new Date(val.event_end_time).getTime() -
              new Date(val.event_start_time).getTime()) /
              60000
          ),
  ...
  let eventStartTime = new Date(
    parseFloat(data?.getEventInfoById?.event_start_time ?? "0") * 1000
  );
  ...
  if (data?.getEventInfoById?.event_start_time) {
    eventStartTime.setMinutes(
      eventStartTime.getMinutes() - eventStartTime.getTimezoneOffset()
    );
    eventEndTime.setMinutes(
      eventEndTime.getMinutes() - eventEndTime.getTimezoneOffset()
    );
    eventEndTime.setMinutes(
      eventEndTime.getMinutes() + data.getEventInfoById.duration_in_minutes
    );
  }
      ...
      {loading || !data ? (
        ...
        <CreateOrEditEventForm
          initialValues={{
            ...
            tags: data.getEventInfoById.tags.map((e) => ({
              label: e,
              value: e,
            })),
          }}
