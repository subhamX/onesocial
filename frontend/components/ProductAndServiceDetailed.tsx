  ...
  return (
    <div className="relative">
      <img
        ...
        className="sm:col-span-2 flex-grow w-full h-60 object-cover border border-blue-200"
      />

      <div className="px-4 pt-4">
        <div className="mt-2 mb-5">
          <div className="mt-2 mb-3 font-bold text-2xl flex items-center gap-2">
            {data.type === ListingType.Service ? (
              <div
                className="tooltip tooltip-right"
                data-tip="This is a digital product offering"
              >
                <ProductIcon className="w-7" />
              </div>
            ) : (
              <div className="tooltip" data-tip="This is a service offering">
        ...
        {!data.is_published && (
          <div className="bg-gray-200 my-2 rounded-lg border text-sm py-4 px-3 justify-start">
            Note: This listing is not published yet on your profile.
          </div>
        )}
        ...
        {data.buy_instance_id === "" && (
          <div className="flex justify-end mt-3">
            <Link href={BUY_LISTING(data.id)}>
              <button className="btn normal-case text-base font-light py-2 md:flex-grow-0 flex-grow btn-primary w-60">
                Get this{" "}
                {data.type === ListingType.DigitalProduct
                  ? "product"
                  : "service"}{" "}
                offering
              </button>
        ...
        {data.buy_instance_id !== "" && (
          <div className="flex gap-2 justify-start flex-col items-start text-sm mt-6 mb-7 py-6 px-4 rounded-md alert-success">
            <div>
              Hurray! 🎉 You&apos;ve got this listing. Please enjoy the product
              / service offering. {data.isAdmin && "(as an admin 😎)"}
            </div>
            ...
            <div className="flex gap-2 flex-col text-sm">
              <div className="font-bold text-base flex min-w-max text-gray-700 items-center gap-1">
                ...
                {data.type === ListingType.DigitalProduct && (
                  <div>
                    Please click on the product items below to download the
                    files.
                  </div>
                )}
                {data.type === ListingType.Service && (
                  <div>The service offering are listed below.</div>
                )}
              </div>
            </div>
          </div>
        ...
        {data.type === ListingType.Service && (
          <div className="mt-2 mb-5">
            <div className="font-bold text-lg mb-1">Included Services</div>
        ...
        {data.type === ListingType.DigitalProduct && (
          <div className="mt-2 mb-5">
            <div className="font-bold text-lg mb-1">Included Product Items</div>

            {data.product_items.map((item, index) => (
              <div
                key={index}
                className="bg-base-200 py-6 px-4 border rounded-md flex gap-3 items-center my-2"
              >
                <div className="avatar">
                ...
                <div>
                  <a
                    target={data.buy_instance_id && "_blank"}
                    rel="noreferrer"
                    href={
                      data.buy_instance_id
                        ? SERVE_PRODUCT_ITEM_FILE(data.buy_instance_id, item.id)
                        : "#"
                    }
                  >
                    <div
        ...
        <div className="text-gray-500 pt-5 font-black">Product Description</div>
