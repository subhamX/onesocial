import {
  ChatBubbleBottomCenterIcon as AnnotationIcon,
  BookOpenIcon,
  BriefcaseIcon,
  HeartIcon,
...
export const EventsDetailedScreen = () => {
  ...
  const handleEventRegister = () => {
    mutateFn({
      ...
      refetchQueries: [
        { query: getEventInfoById, variables: { event_id: eventId } },
      ], // TODO: make this update locally
      onError(error) {
  ...
  const isCurrentLoggedInUserAMember =
    !!data?.authUserEventState?.is_user_a_follower;
      ...
      {event && (
        <div className="max-w-4xl mx-auto break-words">
          ...
          <div className="px-4">
            ...
            <div className="flex flex-col justify-center gap-1 text-sm text-gray-400 pt-1 max-w-sm">
              <div className="grid grid-cols-2 text-sm items-center">
                <div className="font-medium text-gray-500 flex items-center gap-1">
                  <UserCircleIcon className="w-6" /> Created By:
                </div>
              ...
              <div className="grid grid-cols-2 items-center">
                <div className="font-medium text-gray-500 flex items-center gap-1">
                  <LightningBoltIcon className="w-6" /> Start Time:
                </div>
                <div>
                  {dayjs(parseFloat(event.event_start_time) * 1000).format(
              ...
              <div className="grid grid-cols-2 items-center">
                <div className="font-medium text-gray-500 flex items-center gap-1">
                  <ClockIcon className="w-6" /> Duration:
                </div>
                <div>
                  {dayjs
                    .duration({ minutes: event.duration_in_minutes })
                    .humanize()}{" "}
                  (approx)
                </div>
              </div>
              <div className="grid grid-cols-2 items-center">
                <div className="font-medium text-gray-500 flex items-center gap-1">
                  <UserGroupIcon className="w-6" /> Guests:
                </div>
                <div className="flex gap-1 items-center">
                  {event.number_of_registrations === 0
                    ? "None. (Would you be the first one. 🥺)"
            ...
            {isEventAdmin && (
              <Link href={MANAGE_EVENT(event.event_id)}>
            ...
            {isMemberRegistered && authUserEventState?.joining_info ? (
                ...
                <div className="flex gap-2 flex-col text-sm">
                  ...
                  {authUserEventState.joining_info.address && (
                    <div className="flex gap-2 text-gray-600">
                      ...
                      <div>{authUserEventState.joining_info.address}</div>
                    </div>
                  )}
                  {authUserEventState.joining_info.event_url && (
                    <div className="flex gap-2 text-gray-600">
                      <div className="font-bold">Event URL:</div>
                      <div>{authUserEventState.joining_info.event_url}</div>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                {isLoggedIn &&
                  event.is_member_only_event &&
                  !isCurrentLoggedInUserAMember && (
                    <div className="alert alert-warning alert-sm text-sm">
                      Sorry. It&apos;s a members only event. You cannot register
                      for it. 😢
                    </div>
                  )}
            ...
            <div className="text-gray-500 pt-5 font-black">Event Details</div>
