        ...
        >
          <Form>
            ...
            <label className="input-group text-sm font-medium my-2 flex-col sm:flex-row">
              <div className="label justify-start gap-1 w-full pb-1">
                <CalendarIcon className="w-6" /> Start Time
              </div>
              <Field
...
const LocationType = ({ fieldId }: { fieldId: string }) => {
  ...
  return (
    <div className="tabs mb-4 bg-base-100 tabs-boxed">
      <a
        className={"tab gap-2 " + (value == "VIRTUAL" && "tab-active")}
        onClick={() => setValue("VIRTUAL")}
      >
        <DesktopComputerIcon className="w-6" /> <div>Online</div>
      </a>
      <a
        className={"tab gap-2 " + (value == "IN_PERSON" && "tab-active")}
        onClick={() => setValue("IN_PERSON")}
      >
        <GlobeAltIcon className="w-6" /> <div>In Person</div>
      </a>
    </div>
  );
};
  ...
  return (
    <div className="font-medium text-sm bg-base-200 pb-4 pt-2 px-5 border">
      <label className="input-group my-2 flex-col sm:flex-row">
        <div className="label w-full justify-start gap-1 pb-1">
          <LinkIcon className="w-6" /> Event Url{" "}
          {value === "IN_PERSON" && "(optional)"}
        </div>
        <Field
          name="event_url"
          type="url"
      ...
      <label className="input-group my-2 flex-col sm:flex-row">
        <div className="label justify-start gap-1 w-full pb-1">
          <GlobeIcon className="w-6" /> Address{" "}
          {value === "VIRTUAL" && "(optional)"}
        </div>
        <Field
          name="address"
          type="text"
      ...
      <label className="input-group my-2 flex-col sm:flex-row items-start">
        <div className="label justify-start gap-1 w-full pb-1">
          <ShieldCheckIcon className="w-6" /> Additional (Secret Info 🤐)
        </div>
        <Field
      ...
      <div className="alert alert-sm font-semibold text-xs flex-col items-start gap-1">
        <p className="text-gray-400">
          ...
          Important: The following data will be visible to users only after they
          register. So keep the private things like meeting passwords etc in
          here.
        </p>
      </div>
    </div>
