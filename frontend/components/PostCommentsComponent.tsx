...
export const noAuthUserDefaultAvatar =
  "https://unsplash.com/photos/wQLAGv4_OYs/download?ixid=MnwxMjA3fDB8MXxzZWFyY2h8MjB8fHVzZXIlMjBwcm9maWxlJTIwYWJzdHJhY3R8ZW58MHx8fHwxNjYwNTkxNzY2&force=true&w=100";

export const PostCommentsComponent = ({ postId }: { postId: string }) => {
  // fetch user
  const { loading: userDataLoading, data: user } = useQuery<{
  ...
  const [commentMutateFunx] = useMutation<
    { submitPostComment: Mutation["submitPostComment"] },
    MutationSubmitPostCommentArgs
  >(submitPostComment, {
    update(cache, { data }) {
      if (data?.submitPostComment) {
        ...
        const post = cache.readQuery<
          {
            getPostInfoById: Query["getPostInfoById"];
            authUserPostState: Query["authUserPostState"];
          },
          QueryGetPostInfoByIdArgs
        >({
          query: getPostInfoById,
          variables: { post_id: data.submitPostComment.post_id },
        });
  ...
  const { fetchMore, data: postCommentsData } = useQuery<{
    getPostComments: PostComment[];
  }>(getPostComments, {
    variables: {
      offset: 0,
      limit: 3,
    ...
    skip: !postId,
    onCompleted(data) {
      if (data.getPostComments.length < 3) setHasMore(false);
    },
  });
  ...
  return (
    <div className="bg-base-200 mb-10">
      <div className="p-5">
        {hasMore && (
          <div
            className="text-gray-400 font-bold text-xs cursor-pointer mb-4"
            onClick={() => {
              fetchMore({
                variables: {
                ...
                if (e.data.getPostComments.length < 3) setHasMore(false);
              });
            }}
          >
        ...
        {postCommentsData &&
          postCommentsData.getPostComments.map((e, indx) => (
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="bg-base-100 rounded py-2 px-3">
                  <div className="font-medium space-x-2">
                    <span>{e.posted_by.name}</span>{" "}
                    <span className="text-gray-500 text-xs">
                      {dayjs(parseFloat(e.commented_at) * 1000).fromNow()}
                    </span>
                  </div>
            ...
            <div className="flex gap-5 h-10 items-center">
              <div className="avatar">
                <div className="w-10 rounded-full">
                  <img
                    src={
                      user?.getCurrentUser?.avatar_url ??
                      noAuthUserDefaultAvatar
                    }
                  />
                </div>
              </div>
              ...
              <FormInputField
                required
                fieldId="new_comment"
                disabled={user?.getCurrentUser === null}
                placeholder={
