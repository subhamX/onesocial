  ...
  [ListingType.DigitalProduct]: [
    {
      ...
      description:
        "Offer your notion template and make the lives of your users easier.",
  ...
  [ListingType.Service]: [
    {
      ...
      description:
        "Use your domain expertise to solve the problem of the users.",
    ...
    },
    {
      ...
      name: "Resume Reviews",
      description:
        "Review resumes and provide feedback candidates on how to improve them.",
      includes_video_call_support: false,
      includes_chat_support: true,
    },
    {
        ...
        <Formik initialValues={initialValues} onSubmit={handleSubmit}>
          {({ values, setFieldValue }) => (
            <Form>
              ...
              <h2 className="font-extrabold text-2xl mb-7">
                {!isEditMode
                  ? "Add a new product or service offering"
                  : "Edit listed product or service offering"}
              </h2>

              <div className="label font-black mt-5 mb-2 justify-start gap-1 w-full pb-1">
                Choose one of the listing type:
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div
                  ...
                  className={
                    "border border-black cursor-pointer shadow-xl py-4 px-3 " +
                    (ListingType.Service === values.listing_type
                      ? " bg-secondary text-secondary-content  shadow-2xl"
                      : " bg-base-200")
                  }
                >
                  <img
                    src="https://img.icons8.com/color/48/000000/service.png"
                    className="w-6 h-6 mr-2"
                  />
                  <div className="font-bold mt-1 mb-2">Offer Services</div>
                  <div className="text-sm">
                    Help users with interview preparation, hosting a website,
                    setting up a shopify domain, etc.
                  </div>
                </div>
                ...
                <div
                  ...
                  className={
                    "border border-black cursor-pointer shadow-xl py-4 px-3 " +
                    (ListingType.DigitalProduct === values.listing_type
                      ? " bg-secondary text-secondary-content  shadow-2xl"
                      : " bg-base-200")
                  }
                >
                  ...
                  <div className="text-sm">
                    Sell a set of digital files like ebooks, notion template etc
                    that can be downloaded by the user.
                  </div>
                </div>
              </div>

              <div className="label font-bold pb-2 text-gray-600 mt-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-7">
                {listingTypes[values.listing_type].map((e, indx) => (
                  <div
                    onClick={() => {
                      if (indx === activeExampleListing)
                        setActiveExampleListing(-1);
                      else {
                        setActiveExampleListing(indx);
                        setFieldValue("name", e.name);
                        setFieldValue(
                          "includes_video_call_support",
                          e.includes_video_call_support
                        );
                        setFieldValue(
                          "includes_chat_support",
                          e.includes_chat_support
                        );
                      }
                    }}
                    key={indx}
                    className={
                      "border border-black cursor-pointer shadow-xl py-4 px-3 " +
                      (indx === activeExampleListing
                        ? "bg-accent text-accent-content  shadow-2xl"
                        : "bg-gray-200")
                    }
                  >
                    {e.logo && <img src={e.logo} className="w-6 h-6 mr-2" />}
                    <div className="font-semibold text-sm mt-1 mb-2">
                      {e.name}
                    </div>
              ...
              <label className="input-group my-2 flex-col sm:flex-row">
                <div className="label justify-start font-black gap-1 w-full pb-1">
                  Name
                </div>
                <Field
                  name="name"
              ...
              <label className="input-group my-2 flex-col sm:flex-row">
                <div className="label justify-start font-black w-full pb-1">
                  Price
                </div>
              ...
              {values.listing_type === ListingType.Service ? (
                <>
                  <label className="input-group cursor-pointer py-1 grid grid-cols-2 items-center justify-start gap-3">
                    <span className="label pb-1 pl-1 text-sm font-black">
                      Includes chat support
                    </span>
                    <Field
                      type="checkbox"
                      name="includes_chat_support"
                      className="toggle"
                    />
                  </label>
                  ...
                  <label className="input-group cursor-pointer py-1 grid grid-cols-2 items-center justify-start gap-3">
                    <span className="label pb-1 pl-1 text-sm font-black">
                      Includes video call support
                    </span>
                    <Field
                      type="checkbox"
                      name="includes_video_call_support"
                      className="toggle"
                    />
                  </label>

                  <VideoChatLength
                    fieldId="video_duration"
                    videoCallFieldId="includes_video_call_support"
                  />
                </>
              ) : (
                <div className="alert alert-sm bg-gray-200 text-xs font-medium mt-4">
                  Note: In next section you can manage the content files for
              ...
              <div>
                ...
                <div className="tabs mb-2 bg-base-100 tabs-boxed">
                  <a
                    className={
                      "tab gap-2 " + (currentTabId == 0 && "tab-active")
                    }
                  ...
                  <a
                    className={
                      "tab gap-2 " + (currentTabId == 1 && "tab-active")
                    }
              ...
              <div className="font-bold">
                <FormMultiSelectAsyncField
                  loadOptions={async (val) => {
                    ...
                    return data.getListingTags.map((e) => ({
                      value: e,
                      label: e,
                    }));
                  }}
              ...
              <button className="btn mt-7 mb-2" type="submit">
                {values.listing_type === ListingType.DigitalProduct
                  ? "Next"
                  : "Publish"}
              </button>
            </Form>
...
const VideoChatLength = ({
  fieldId,
  videoCallFieldId,
}: {
  fieldId: string;
  videoCallFieldId: string;
}) => {
  ...
  return (
    <div>
      {value && (
        <div className="text-sm text-center flex gap-2 flex-wrap">
          <div className="label justify-start font-black gap-1 w-full pb-1">
            Call Duration
          </div>
          ...
          {videoDuration.map((e, indx) => (
            <div
              onClick={() => setVideoDurationOptionIndx(indx)}
              key={indx}
              className={
                "cursor-pointer px-1 py-2 my-2 w-20 rounded-lg " +
                (videoDurationOptionIndx == indx ? "bg-accent" : "bg-slate-300")
              }
            >
              {e}
            </div>
          ))}
          {videoDurationOptionIndx + 1 == videoDuration.length && (
            <div className="w-full">
              <label className="input-group my-2 flex-col sm:flex-row">
                <div className="label justify-start font-black gap-1 w-full pb-1 text-gray-500">
